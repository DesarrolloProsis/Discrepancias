...
namespace NuevoFormatoDiscrepancias.Servicios
{
    ...
    {
        public List<filas> Datos_Tabla_4(List<string> Todos_Carriles, List<string> Carriles, DateTime Fecha, string Turno, string Plaza)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < Prueba.Count; i++)
                {
                    for (int _Turno = 1; _Turno < 4; _Turno++)
                    {
                        ...
                        newfila.Eficiencia_Dia_Pre = Eficiencia_Dia_Pre(Prueba, i, _Turno);
                        newfila.Discrepancias_Post = Discrepancias_Post(Prueba, i, _Turno);
                        newfila.Eficiencia_Post = Eficiencia_Post(Prueba, i, _Turno);
                        newfila.Eficiencia_Dia_Post = Eficiencia_Dia_Post(Prueba, i, _Turno);
